...
export class UserService {
  ...
  async createManager(createUserDto: CreateUserDto) {
    ...
    try {
      ...
      if (userError) {
        ...
        throw new Error(
          `Gagal menambahkan data pengguna: ${userError.message}`
        );
      }
  ...
  async deleteManager(id: string) {
    ...
    try {
      ...
      if (authError) {
        ...
        await supabaseClient.from('users').insert([existingUser]);
  ...
  async deleteStaff(managerId: string | null, staffId: number) {
    try {
      ...
      if (userOutletError) {
        throw new Error(
          `Failed to delete user_outlet: ${userOutletError.message}`
        );
      }
  ...
  async createStaff(createUserDto: CreateUserDto, user: User | undefined) {
    const supabaseClient = this.supabaseService.getClient();
    const { email, password, fullname, address, phone, photo_url } =
      createUserDto;
    ...
    try {
      ...
      if (authError) {
        throw new Error(
          `Gagal mendaftarkan user di Auth: ${authError.message}`
        );
      }
      ...
      if (user && user.role === 'manager') {
        ...
        const { data: managerOutlet, error: managerError } =
        ...
        if (userOutletError) {
          throw new Error(
            `Gagal menambahkan relasi ke user_outlet: ${userOutletError.message}`
          );
        }
      }
  ...
  async addUserToOutlet(addUserToOutletDto: AddUserToOutletDto) {
    ...
    try {
      ...
      const { data: userOutlet, error: userOutletError } = await supabaseClient
        ...
        .insert([
          {
            user_id: addUserToOutletDto.user_id,
            outlet_id: addUserToOutletDto.outlet_id,
          },
        ])
        .select();
      ...
      if (userOutletError) {
        throw new Error(
          `Gagal menambahkan user ke outlet: ${userOutletError.message}`
        );
      }
