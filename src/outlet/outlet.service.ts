    ...
    try {
      ...
      if (accountError) {
        throw new BadRequestException(
          `Gagal membuat akun manager: ${accountError.message}`
        );
      }
      ...
      const {
        data: outletData,
        error: outletError,
      }: PostgrestSingleResponse<Outlet> = await supabaseClient
        .from('outlets')
        .insert([
          {
      ...
      if (outletError) {
        ...
        throw new BadRequestException(
          `Gagal membuat outlet: ${outletError.message}`
        );
      }
      ...
      const {
        data: managerData,
        error: managerError,
      }: PostgrestSingleResponse<User> = await supabaseClient
        .from('users')
        .insert([
          {
      ...
      if (managerError) {
        ...
        await supabaseClient.auth.admin.deleteUser(accountData.user?.id ?? '');
        throw new BadRequestException(
          `Gagal membuat manager: ${managerError.message}`
        );
      }

      // 4. Insert user_outlet
      const { error: userOutletError }: PostgrestSingleResponse<User> =
      ...
      if (userOutletError) {
        ...
        await supabaseClient.auth.admin.deleteUser(accountData.user?.id ?? '');
        throw new BadRequestException(
          `Gagal membuat user_outlet: ${userOutletError.message}`
        );
      }
